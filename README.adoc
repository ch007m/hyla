...
slideshow presentation using as backend (DeckJS, DZSlides, ...) later on
    ...
    hyla generate --p my-project --d ~/Temp/MyBlankProject/  --r toc2html --toc ~/MyProjects/asciidoctor-hyla/data/toc.adoc
...
=== Generate HTML pages from Asciidoc
    ...
    hyla generate --s ~/Temp/MyTrainingProject/ --d ~/Temp/MyTrainingProject/generated_content/ --r adoc2html
